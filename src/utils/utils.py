...
class Preprocessor:
    ...
    def Preprocess(self) -> None:
        ...
        csvLog = pd.read_csv(
            filepath_or_buffer=os.path.realpath(
                self.inputPath + self.fileName
            ),
        ...
        if "Timestamp" in list(columnNewcolumn.values()):
            ...
            logSamples.to_csv(
                self.outputPath + self.outputFileName, index=False
            )
        else:
            print(
                "Choose right datafram, This type of table don't follow MPMM rule"
            )
...
class EventGraphBuilder:
    def __init__(self, password, filename):
        ...
        self.filePath = os.path.realpath(
            "./data/processed/" + f"{filename}.csv"
        )  # Acess for the processed data
        self.csv_data_set = pd.read_csv(
            self.filePath
        )  # Load the processed data as datafram object
        self.event_attributes = self.csv_data_set.columns

        self.data_entities = ["Case", "Actor"]

    def Builder(self):
        query_create_event_nodes = (
            f"USING PERIODIC COMMIT LOAD CSV WITH HEADERS "
            f'FROM "file:///{self.filePath}" as line'
        )
